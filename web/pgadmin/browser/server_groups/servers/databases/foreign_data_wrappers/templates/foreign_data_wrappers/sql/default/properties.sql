...
SELECT fdw.oid as fdwoid, fdwname as name, fdwhandler, fdwvalidator, description,
    array_to_string(fdwoptions, ',') AS fdwoptions, sys_get_userbyid(fdwowner) as fdwowner, array_to_string(fdwacl::text[], ', ') as acl,
    CASE
...
LEFT OUTER JOIN sys_proc vh on vh.oid=fdwhandler
LEFT OUTER JOIN sys_proc vp on vp.oid=fdwvalidator
LEFT OUTER JOIN sys_namespace vh_nsp ON vh_nsp.oid=vh.pronamespace
LEFT OUTER JOIN sys_namespace vp_nsp ON vp_nsp.oid=vp.pronamespace

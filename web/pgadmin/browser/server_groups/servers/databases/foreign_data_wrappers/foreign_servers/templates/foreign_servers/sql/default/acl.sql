...
FROM
    ...
    FROM
        (SELECT srvacl FROM sys_foreign_server fsrv
            LEFT OUTER JOIN sys_shdescription descr ON (
            fsrv.oid=descr.objoid AND descr.classoid='sys_foreign_server'::regclass)
{% if fsid %}
...
{% endif %}
        ...
        FROM (SELECT aclexplode(srvacl) as d FROM sys_foreign_server fsrv1
        {% if fsid %}

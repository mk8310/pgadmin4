...
FROM
    ...
    AND (
      ...
      OR nsp.nspname = 'sys_catalog'
    )
{% if add_serials %}
{# Here we will add serials types manually #}
    UNION SELECT 'smallserial', 0, 2, 'b', 0, 'sys_catalog', false, false
    UNION SELECT 'bigserial', 0, 8, 'b', 0, 'sys_catalog', false, false
    UNION SELECT 'serial', 0, 4, 'b', 0, 'sys_catalog', false, false
{% endif %}) AS dummy

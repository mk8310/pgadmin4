SELECT rel.oid, rel.relname AS name, rel.reltablespace AS spcoid,rel.relacl AS relacl_str,
  ...
  sys_get_userbyid(rel.relowner) AS relowner, rel.relhasoids, rel.relkind,
  ...
  (SELECT count(*)
	...
	CASE WHEN typ.typname IS NOT NULL THEN (select quote_ident(nspname) FROM sys_namespace WHERE oid = {{scid}}::oid )||'.'||quote_ident(typ.typname) ELSE typ.typname END AS typname,
    ...
    {% if tid %}, (CASE WHEN rel.relkind = 'p' THEN sys_get_partkeydef({{ tid }}::oid) ELSE '' END) AS partition_scheme {% endif %}
FROM sys_class rel

SELECT
    ...
    sys_relation_size({{ exid }}::OID) AS {{ conn|qtIdent(_('Index size')) }}
{#=== Extended stats ===#}
{% if is_pgstattuple %}
...
FROM
    pgstatindex('{{conn|qtIdent(schema)}}.{{conn|qtIdent(name)}}'), sys_stat_all_indexes stat
{% else %}
FROM
    sys_stat_all_indexes stat
{% endif %}
    JOIN sys_statio_all_indexes statio ON stat.indexrelid = statio.indexrelid
    JOIN sys_class cl ON cl.oid=stat.indexrelid

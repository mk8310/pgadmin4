SELECT rel.oid, rel.relname AS name,
    ...
    (SELECT count(*) FROM sys_trigger WHERE tgrelid=rel.oid AND tgisinternal = FALSE AND tgenabled = 'O') AS has_enable_triggers,
...
FROM
    (SELECT * FROM sys_inherits WHERE inhparent = {{ tid }}::oid) inh
    LEFT JOIN sys_class rel ON inh.inhrelid = rel.oid

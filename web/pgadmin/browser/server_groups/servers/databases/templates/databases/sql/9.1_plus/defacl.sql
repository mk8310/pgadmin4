...
FROM
    ...
    FROM
        (SELECT defaclobjtype, aclexplode(defaclacl) as acl FROM sys_catalog.sys_default_acl dacl
      WHERE dacl.defaclnamespace = 0::OID) d) a
    LEFT JOIN sys_catalog.sys_roles g ON (a.grantor = g.oid)
    LEFT JOIN sys_catalog.sys_roles gt ON (a.grantee = gt.oid)
